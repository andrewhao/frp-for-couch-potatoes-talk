...
/***/ function(module, exports, __webpack_require__) {
	...
	var mouseVelocityStream = mouseMoveStream.timestamp().pairwise().map(function (coordinatePair) {
	  var _coordinatePair = _slicedToArray(coordinatePair, 2);

	  var first = _coordinatePair[0];
	  var last = _coordinatePair[1];

	  var deltaTime = (last.timestamp - first.timestamp) / 1000;
	  var deltaY = last.value.y - first.value.y;
	  return deltaY / deltaTime;
	});

	mouseVelocityStream.subscribe(function (velocity) {
	  (0, _jquery2.default)('.velocity-output').text('Mouse velocity is: ' + velocity + ' pixels/s');
	});

	var mouseAccelStream = mouseVelocityStream.timestamp().pairwise().map(function (coordinatePair) {
	  var _coordinatePair2 = _slicedToArray(coordinatePair, 2);

	  var first = _coordinatePair2[0];
	  var last = _coordinatePair2[1];

	  var deltaTime = (last.timestamp - first.timestamp) / 1000;
	  var deltaVelocity = last.value - first.value;
	  return deltaVelocity / deltaTime;
	});

	mouseAccelStream.subscribe(function (acceleration) {
	  (0, _jquery2.default)('.acceleration-output').text('Mouse acceleration is: ' + acceleration + ' pixels/s^2');
	});

	var accelDirectionChangeStream = mouseAccelStream.pairwise().map(function (values) {
